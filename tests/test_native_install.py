...
class InstallRenderingTests(unittest.TestCase):
    def test_provider_settings_stay_in_right_table(self):
        result = tomllib.loads(render_config(BASE, 'aimai1', {'port': 12345, 'catalog_file': 'C:\\path\\models.json'}, 'test-not-real-secret'))
        provider = result['model_providers']['aimai1']
        ...
        self.assertEqual(result['model_provider'], 'aimai1')
    ...
    def test_existing_auth_headers_require_explicit_merge(self):
        ...
        with self.assertRaises(ValueError): render_config(text, 'aimai1', {'port': 12345, 'catalog_file': 'new'}, 'test')
